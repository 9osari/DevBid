...
### 동시성 제어
다수 사용자가 동시에 입찰할 때 데이터 정합성을 보장하기 위해 Redis 분산락과 JPA Optimistic Lock을 함께 적용했습니다. 락 적용 과정에서 발생한 데드락 문제는 트랜잭션과 락의 계층을 분리하여 해결했습니다.
...
### 실시간 통신
WebSocket과 STOMP를 활용해 입찰 현황을 실시간으로 브로드캐스트합니다. 멀티 서버 환경에서 모든 접속자에게 메시지를 전파하기 위해 Redis Pub/Sub을 도입했습니다.
